...
function AuthContent() {
  ...
  const redact = async () => {
    ...
    try {
      ...
      const r = await axios.post(
        'https://d183-223-190-80-150.ngrok-free.app/redact',
        JsonData
      );
      const data = r.data;
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10">
      <h2 className="my-3 text-3xl font-bold">PIReT for images</h2>
      <input
        type="file"
        accept="image/*"
        onChange={handleImageChange}
        className="w-5/6  px-3 h-16 py-2 text-white bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-gray-500 "
      />
      ...
      <div className="relative  w-5/6">
        ...
        <button
          className="px-4 text-sm my-3 py-2 bg-gray-300 bg-blend-normal text-black rounded-lg z-20 relative float-right hover:bg-gray-200"
          onClick={toggleFilter}
        >
          Filters
        </button>
        {isFilterOpen && (
          <div className="absolute flex max-h-80 overflow-y-auto border bg-black p-6 rounded shadow-lg z-30 bg-opacity-50 space-x-10">
            {Object.keys(filters).map((category) => {
              const isAllSelected = filters[category].every((option) =>
                selectedFilters.includes(option)
              );
              return (
                <div
      ...
      <div className="w-5/6 my-2 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg relative">
        {selectedOption === 'text' && !isLoading ? (
          ...
          <p className="text-white font-mono break-all">
            Redacted Data will appear here...
          </p>
        )}
        {(redactedText || image) && (
          <button
